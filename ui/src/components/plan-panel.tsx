...
export function PlanPanel({ className, steps: stepsProp = [] }: PlanPanelProps) {
  ...
  return (
    <div className={cn('runner-panel', className)}>
      {/* 折叠状态 */}
      {!isExpanded && <div
        className="flex flex-row items-start justify-between pr-3 relative clickable cursor-pointer rounded-md"
        onClick={togglePanel}
      >
        {/* 左侧的最新计划 */}
              ...
              <div className="max-h-[min(calc(100vh-360px),400px)] overflow-y-auto">
                ...
                >
                  {step.status === 'completed' ? (
                    <Check size={16} className="relative top-0.5 flex-shrink-0 text-emphasis" />
                  ) : (
