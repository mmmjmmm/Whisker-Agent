# MoocManus Agent Skills Core Chain Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 MoocManus 中实现从设置页 Skill ZIP 管理，到 Agent 元数据发现、load_skill 渐进加载、完整包同步至会话沙箱，再到 references/assets/scripts 按需使用的完整最小核心链路。
...
- api/app/domain/services/skills/__init__.py
...
- api/app/infrastructure/external/skill_bundle_storage/__init__.py
...
- Create: api/app/domain/services/skills/__init__.py
...
- Create: api/app/infrastructure/external/skill_bundle_storage/__init__.py
...
IUnitOfWork 增加 skill: SkillRepository；DBUnitOfWork.__aenter__ 初始化 DBSkillRepository。
...
from_domain、to_domain、update_from_domain 完全映射 Skill 的七个业务字段和两个时间字段。DBSkillRepository 使用 select/order_by(SkillModel.name)、enabled.is_(True)、id/name 精确匹配、现有记录更新和 delete。
...
def test_runtime_syncs_full_bundle_once_for_concurrent_loads() -> None:
    async def scenario() -> None:
        ...
        assert first.skill_dir == "/home/ubuntu/.mooc-manus/skills/skill-id/content/demo"
...
SkillRuntime.__init__ 将 snapshots 按 name 建立只读查找，并为每个 id 建 asyncio.Lock、_synced_dirs 字典；同时提供以下只读属性供 Flow 判断，不让 Flow 读取内部字典：
    ...
    async def scenario() -> None:
        runtime = FakeSkillRuntime(
            LoadedSkill(
                ...
                skill_dir="/home/ubuntu/.mooc-manus/skills/id/content/demo",
            )
        )
...
BaseAgent.__init__ 新增 system_prompt_suffix: str = ""，不得修改类级常量：
...
AgentService.__init__ 增加 skill_registry: SkillRegistry 并保存。_create_task 在构造 AgentTaskRunner 前执行：
...
PlannerReActFlow.__init__ 新增 skill_runtime。先组装现有业务工具，再按是否存在快照分别创建独立工具：
