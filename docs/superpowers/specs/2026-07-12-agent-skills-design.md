# WhiskerAgent Agent Skills 核心链路设计
...
WhiskerAgent 将按照开放的 Agent Skills 规范实现一条完整但克制的 Skill 垂直链路：设置页操作者上传 Skill ZIP，`SkillRegistry` 持久化并管理 Skill，Agent 在任务开始时获得已启用 Skill 的轻量元数据目录，并通过 `load_skill(name)` 按需加载完整 `SKILL.md`。首次加载时，系统把完整 Skill 包同步到当前会话沙箱；随后 Agent 使用现有 File 和 Shell 工具按需读取 `references/`、使用 `assets/`、执行 `scripts/`。
...
WhiskerAgent 不是纯本地 CLI，而是 FastAPI 服务加临时任务沙箱。纯文件扫描无法覆盖设置页上传和跨任务持久化；直接使用 OpenAI 托管 Skills 又会破坏当前 DeepSeek/OpenAI-compatible 模型兼容和自有沙箱。因此首期采用项目原生的 `SkillRegistry + SkillRuntime + SkillTool`，同时复用现有 PostgreSQL、OSS、Agent、Tool、Memory、Sandbox、SSE 和 UI 设置框架。
...
```text
/home/ubuntu/.whisker-manus/skills/{skill_id}/bundle.zip
/home/ubuntu/.whisker-manus/skills/{skill_id}/content/
```
...
Skill directory: /home/ubuntu/.whisker-manus/skills/{id}/content/{root_path}
